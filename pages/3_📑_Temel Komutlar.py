...
def page_2():
    with st.sidebar:
        st.title("Yeni başlayan birisinin bilmesi gerekn komutlar ve kullanımları:")
    ...
    with st.expander("Kendinizi Tanıtın"):
        st.markdown(p2_desc)
        st.text(p2_example1)
    ...
    with st.expander("git clone"):
        st.markdown(p4_desc)
        st.code(code_clone, language='bash')

    with st.expander("git add"):
        st.markdown(p5_desc)
        st.text(p5_example1)
    ...
    with st.expander("git branch (Dallar Arası İşlemler)"):
        st.markdown(p8_desc)
        st.write(p8_t1)
    ...
    with st.expander("remote repo (GitHub)"):
        st.markdown(p9_t1)
    ...
    with st.expander("git pull / git fetch"):
        st.markdown(p11_t1)
